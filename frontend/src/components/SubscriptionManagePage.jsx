...
export function SubscriptionManagePage() {
  ...
  useEffect(() => {
    async function fetchList() {
      try {
        const res = await fetch(`/api/subscriptions/list?userId=${userId}`);
        if (!res.ok) throw new Error("구독 목록을 불러올 수 없습니다.");
...
const styles = {
  ...
  header: { marginBottom: "2rem", textAlign: "left" },
  title: { fontSize: "22px", fontWeight: 500, margin: 0, textAlign: "left",color: "#ffffff"},
  subtitle: { fontSize: "13px", color: "#717182", marginTop: "4px", textAlign: "left" },
  grid: {
    display: "grid",
  ...
  name: { fontSize: "15px", fontWeight: 500, margin: 0, textAlign: "left" },
  date: { fontSize: "13px", color: "#717182", margin: "2px 0 0", textAlign: "left" },
  renewBtn: {
    padding: "0.5rem 1rem",
    background: "#00d4ff",
    color: "#0d0d14",
